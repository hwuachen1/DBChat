import os
import json
import pandas as pd
import openai
from utils import *
from jobsearch import *
import streamlit as st
from langchain.vectorstores import Chroma
from langchain.embeddings import OpenAIEmbeddings
from prompts import human_template, system_message
from render import user_msg_container_html_template, bot_msg_container_html_template

LLM_COMPLETION_MODEL = "gpt-3.5-turbo-0613"
LLM_EMBEDDING_MODEL = "all-MiniLM-L6-v2"

# Set OpenAI API key
openai.api_key = st.secrets["OPENAI_API_KEY"]

st.header("DBChat: An AI agent with Multiple Data Sources")

# Initialize embeddings
embeddings = OpenAIEmbeddings()

# Load the vector databases
embedding_function = SentenceTransformerEmbeddings(model_name="all-MiniLM-L6-v2")
handbookDB = Chroma(persist_directory=os.path.join('db', 'handbook'), embedding_function=embedding_function)

# Initialize session state for chat history
if "history" not in st.session_state:
    st.session_state.history = []

# Construct messages from chat history
def construct_messages(history):
    messages = [{"role": "system", "content": system_message}]
    
    for entry in history:
        role = "user" if entry["is_user"] else "assistant"
        messages.append({"role": role, "content": entry["message"]})
    
    # Ensure total tokens do not exceed model's limit
    messages = ensure_fit_tokens(messages)
    
    return messages



def handbook_handler(query):
    print("Using DB Handbook handler...")

    # Get relevant documents from Handbook's database
    relevant_docs = handbookDB.similarity_search(query)
    
    # Use the provided function to prepare the context
    context = get_page_contents(relevant_docs)
    # context = relevant_docs[0].page_content

    # Prepare the prompt for GPT-3.5-turbo with the context
    query_with_context = human_template.format(query=query, context=context)

    return {"role": "user", "content": query_with_context}

# --------------------------------------------------------------
# Use OpenAI’s Function Calling Feature
# --------------------------------------------------------------

function_descriptions = [
    {
        "name": "search_job_deutschebank",
        "description": "Find job position at Deutschebank",
        "parameters": {
            "type": "object",
            "properties": {
                "career_level": {
                    "type": "string",
                    "description": "The career level of the job, e.g. Analyst, Associate, Director ,Vice President",
                },
                "cityname": {
                    "type": "string",
                    "description": "The city where the job is located, e.g. New York, London, Sydney, Mumbai",
                },
            },
            "required": ["career_level", "cityname"],
        },
    }
]


def search_job_handler(query):
    print("Using DB search job handler...")

    # Extract parameters from users' prompt
    completion = openai.ChatCompletion.create(
        model=LLM_COMPLETION_MODEL,
        messages=[{"role": "user", "content": query}],
        # Add function calling
        functions=function_descriptions,
        function_call="auto",  # specify the function call
    )
    output = completion.choices[0].message
    career_level = json.loads(output.function_call.arguments).get("career_level")
    cityname = json.loads(output.function_call.arguments).get("cityname")
    params = json.loads(output.function_call.arguments)
    chosen_function = eval(output.function_call.name)
    print(f"search_job_handler: chosen_function = {chosen_function}")

    context = chosen_function(**params)
    print( f"search_job_handler: context = {context}")
    
    return {"role": "function", "name": output.function_call.name, "content": context}

def other_handler(query):
    print("Using other handler...")
    # Return the query in the appropriate message format
    return {"role": "user", "content": query}


# Function to route query to correct handler based on category
def route_by_category(query, category):
    if category == "0":
        # return career_handler(query)
        return search_job_handler(query)
    elif category == "1":
        return handbook_handler(query)
    elif category == "2":
        # return fa_handler(query)
         return other_handler(query)
    elif category == "3":
        return other_handler(query)
    else:
        raise ValueError("Invalid category")

# Function to generate response
def generate_response():
    # Append user's query to history
    st.session_state.history.append({
        "message": st.session_state.prompt,
        "is_user": True
    })
    
    # Classify the intent
    category = intent_classifier(st.session_state.prompt)
    print(f"category = {category}")
    
    # Route the query based on category
    new_message = route_by_category(st.session_state.prompt, category)
        
    # Construct messages from chat history
    if (category != "0" ):
        messages = construct_messages(st.session_state.history)
    else:
        messages = []
    
    # Add the new_message to the list of messages before sending it to the API
    messages.append(new_message)
    
    # Ensure total tokens do not exceed model's limit
    messages = ensure_fit_tokens(messages)
    
    print(f"\n\nnew messages = {messages}")

    # Call the Chat Completions API with the messages
    response = openai.ChatCompletion.create(
        model=LLM_COMPLETION_MODEL,
        messages=messages,
        functions=function_descriptions,
    )

    # Extract the assistant's message from the response
    assistant_message = response['choices'][0]['message']['content']
    
    # Append assistant's message to history
    st.session_state.history.append({
        "message": assistant_message,
        "is_user": False
    })


# Take user input
st.text_input("Enter your prompt:",
              key="prompt",
              placeholder="e.g. 'How can I diversify my portfolio?'",
              on_change=generate_response
              )

# Display chat history
for message in st.session_state.history:
    if message["is_user"]:
        st.write(user_msg_container_html_template.replace("$MSG", message["message"]), unsafe_allow_html=True)
    else:
        st.write(bot_msg_container_html_template.replace("$MSG", message["message"]), unsafe_allow_html=True)

# query = "I am looking for Analyst job in Deutsche Bank. Is there avaialble Analyst position in Cary?"
# messages = [{"role": "system", "content": system_message}]
# new_message = route_by_category(query, "0")
# messages.append(new_message) 
# messages = ensure_fit_tokens(messages)
# print(f"\n\nnew messages = {messages}")
# response = openai.ChatCompletion.create(
#     model=LLM_COMPLETION_MODEL,
#     messages=messages,
#     functions=function_descriptions,
# )
# assistant_message = response['choices'][0]['message']['content']
# print(f'assistant_message= {assistant_message}')